...
export default function TryOnWizard() {
  ...
  const setPhoto = (file: File) => { clearPhoto(); setRequestError(null); fileRef.current = file; const url = URL.createObjectURL(file); urlRef.current = url; dispatch({ type: "set-photo", previewUrl: url }); };
  const create = async () => { if (!fileRef.current || !state.productId || !state.mask) return; setRequestError(null); try { const form = new FormData(); form.set("room", fileRef.current); form.set("productId", state.productId); form.set("mask", JSON.stringify(state.mask)); const response = await fetch("/api/try-on/jobs", { method: "POST", body: form }); if (!response.ok) { const message = response.status === 413 ? "Фото слишком большое для отправки." : response.status === 411 ? "Сервер просит повторить отправку. Фото и область сохранены." : response.status === 503 ? "Сервис временно недоступен или занят." : "Проверьте фото и выбранную область."; throw new Error(message); } const job = await response.json() as TryOnJobView; if (!job.id) throw new Error("Сервис вернул неполный ответ."); setRequestError(null); dispatch(state.step === "generating" && state.error ? { type: "retry-job-created", jobId: job.id } : { type: "job-created", jobId: job.id }); } catch (error) { setRequestError(error instanceof Error ? error.message : errorText.provider_failed); } };
  ...
  const goBack = () => { if (state.step === "photo") clearPhoto(); setRequestError(null); dispatch({ type: "back" }); };
