...
export default function ProductStep({ onSelect }: { onSelect: (productId: string) => void }) {
  ...
  return <section className="mx-auto w-full max-w-3xl px-4 py-8">
    ...
    <div className="mt-6 grid grid-cols-2 gap-3 sm:grid-cols-4" role="group" aria-label="Категории мебели">
      {TRY_ON_GROUPS.map((group) => <button key={group.id} type="button" onClick={() => setGroupId(group.id)} aria-pressed={groupId === group.id} className={`min-h-11 rounded-lg border px-3 text-sm font-medium focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-primary ${groupId === group.id ? "border-primary bg-primary text-primary-foreground" : "bg-background hover:bg-secondary"}`}>{group.label}</button>)}
    </div>
